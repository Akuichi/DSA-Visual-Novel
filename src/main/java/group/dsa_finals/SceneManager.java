...
public class SceneManager {
    ...
    private void LoadStoryData() { //Load lang yung JSON for our story data
        ...
        } catch (IOException e) { //Just to catch errors
    ...
    private void DisplayScene() {
        ...
        if (sceneNode == null) {  //If blank yung scenenode for some reason may mali sa JSON
        ...
        _typingThread = new Thread(() -> {
            try {
                ...
                _option1Button.setVisible(false); 
                _option2Button.setVisible(false);
                for (int i = 0; i < dialogueText.length(); i++) { //Simple for loop to print characters of string 1 by 1
        ...
        try {
            ...
            if (imgFile.exists()) { //Check kung may image file ba, if yes continue
    ...
    private void LoadDialogueButtons() {
        ...
        if (optionsNode.has("1")) { //For button 1
        ...
        if (optionsNode.has("2")) { //For button 2
